...
# Đường dẫn dữ liệu Cityscapes
img_dir = r"F:\StudyatCLass\Study\class\Thigiacmaytinh\Citispaces_Segmentation_Random_Forest\DataCitiSpaces\train\img"  # Thay bằng đường dẫn thực tế
label_dir = r"F:\StudyatCLass\Study\class\Thigiacmaytinh\Citispaces_Segmentation_Random_Forest\DataCitiSpaces\train\label"  # Thay bằng đường dẫn thực tế
...
def extract_features(img_path, label_path):
    ...
    img_gray = cv2.resize(img_gray, (IMG_WIDTH, IMG_HEIGHT))  # width x height
    ...
    kp = [cv2.KeyPoint(x, y, 20) for y in range(0, img_gray.shape[0], 20) 
                               for x in range(0, img_gray.shape[1], 20)]
    _, dense_sift = sift.compute(img_gray, kp)
    if dense_sift is None:
        dense_sift = np.zeros((len(kp), 128))  # Fallback nếu không tìm thấy keypoints
...
def process_pixel_data(images, labels):
    """Xử lý dữ liệu pixel-level và lấy mẫu"""
    pixel_features = []
    pixel_labels = []
    
    for img_features, label_img in zip(images, labels):
        # Reshape label về dạng 2D nếu cần
        if label_img.ndim == 1:
            label_img = label_img.reshape(IMG_WIDTH, IMG_HEIGHT)
        
        num_pixels = label_img.size
        repeated_features = np.tile(img_features, (num_pixels, 1))
...
print("Đang tải và xử lý dữ liệu Cityscapes...")
start_time = time()
...
for img_name, label_name in tqdm(zip(img_files, label_files), total=len(img_files)):
    ...
    try:
        ...
        labels.append(label.flatten())  # Làm phẳng nhãn để lưu trữ
...
plt.title('Phân bố pixel theo lớp trong tập dữ liệu')
plt.xlabel('ID Lớp')
...
try:
    ...
    print(f"Lỗi khi xử lý dữ liệu pixel: {str(e)}")
    exit()
...
X = sampled_pixels[:, :-1]  # Đặc trưng
y = sampled_pixels[:, -1]   # Nhãn
...
print(f"Độ chính xác tốt nhất: {best_score:.4f}")
...
def display_segmentation_results(img_dir, label_dir, model, num_images=3):
    ...
    for img_name in test_images:
        ...
        for i in range(h):
            for j in range(w):
                ...
                pixels[i*w + j] = np.concatenate([
                    [img_gray[i,j]/255.0],
                    np.zeros(IMG_SIZE*IMG_SIZE - 1),  # Giữ chỗ cho các đặc trưng khác
                    np.zeros(IMG_SIZE*IMG_SIZE),     # HOG image
        ...
        true_label = cv2.resize(true_label, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_NEAREST)
